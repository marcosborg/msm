...
class ClientsController extends Controller
{
    ...
    public function store(StoreClientRequest $request)
    {
        ...
        foreach ($request->input('logos', []) as $file) {
            $client->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('logos');
        }
    ...
    public function update(UpdateClientRequest $request, Client $client)
    {
        ...
        if (count($client->logos) > 0) {
            foreach ($client->logos as $media) {
                if (! in_array($media->file_name, $request->input('logos', []))) {
                    $media->delete();
                }
            }
        }
        $media = $client->logos->pluck('file_name')->toArray();
        foreach ($request->input('logos', []) as $file) {
